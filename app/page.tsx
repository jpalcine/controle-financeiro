            ...
            {monthFixedBills.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {monthFixedBills.map((item) => (
                  <div key={item.id} className="rounded-2xl border border-slate-200 p-3.5 transition hover:bg-slate-50 sm:p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-[15px] font-semibold text-slate-900 sm:text-base">{item.description}</p>
                        <div className="mt-1 flex flex-wrap gap-1.5 text-xs text-slate-500 sm:text-sm">
                        ...
                        <p className="mt-1 line-clamp-1 text-xs text-slate-500 sm:text-sm">
                          Início em {formatMonthLabel(item.startMonth)}{item.notes ? ` • ${item.notes}` : ""}
                        </p>
                      </div>

                      <div className="shrink-0 text-right">
                        <p className="text-sm font-bold text-slate-900 sm:text-base">{formatCurrency(item.amount)}</p>
                        <span className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${item.monthStatus === "pago" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto_auto]">
                      <button
                        ...
                        className={`rounded-xl px-3 py-2.5 text-sm font-medium transition ${item.monthStatus === "pago" ? "bg-amber-50 text-amber-700 hover:bg-amber-100" : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100"}`}
                      >
                        {item.monthStatus === "pago" ? "Voltar para pendente" : "Marcar como pago"}
                      </button>
                      <div className="grid grid-cols-2 gap-2 sm:flex sm:gap-2">
                        <button onClick={() => openEditFixedBill(item)} className="rounded-xl bg-slate-100 px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-200">Editar</button>
                        <button onClick={() => handleDeleteFixedBill(item.id)} className="rounded-xl bg-rose-50 px-3 py-2.5 text-sm font-medium text-rose-600 transition hover:bg-rose-100">Excluir</button>
                      </div>
                    </div>
                  </div>
                ))}
